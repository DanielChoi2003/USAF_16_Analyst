...
async function buildDirectReport({ pythonExec, tempFilePath, reason, execCommandImpl = execCommand }) {
  ...
  const fallbackResult = await execCommandImpl(fallbackCommand);
...
function createApp(options = {}) {
  ...
  const {
    ...
    searchPackageEventsImpl = searchPackageEvents,
    resultsDir = path.join(__dirname, '..', 'analysis_results'),
    inputsDir = path.join(__dirname, '..', 'inputs'),
    pythonExec = process.env.PYTHON_PATH || 'python3',
    useRagPrimary = process.env.USE_RAG_PRIMARY === 'true',
  } = options;
  ...
  app.post('/misp-analyze', (req, res) => {
    ...
    fs.writeFile(tempInputPath, JSON.stringify(req.body, null, 2), (err) => {
      ...
      const mispPythonExec = options.mispPythonExec || process.env.PYTHON_PATH || 'python';
      const command = `${shellEscape(mispPythonExec)} ${shellEscape(pythonScriptPath)} ${shellEscape(tempInputPath)} ${shellEscape(tempOutputPath)}`;
  ...
  app.post('/analyze', (req, res) => {
    ...
    const analyze = async () => {
      ...
      try {
        elasticEvents = await searchPackageEventsImpl(original_input);
      } catch (error) {
      ...
      fs.writeFile(tempFilePath, JSON.stringify(llmContext, null, 2), async (err) => {
        ...
        try {
          if (useRagPrimary) {
            ...
            const ragResult = await execCommandImpl(command);
