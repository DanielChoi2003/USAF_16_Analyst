...
def build_prompt(payload: dict, reason: str) -> str:
    ...
    return (
        ...
        "- Key Findings: Use only the number of bullet points justified by the evidence. Focus on observed evidence, affected hosts/users, suspicious behavior, and confidence caveats. Do not pad the section to hit a target count.\n"
        "- MITRE ATT&CK Assessment: Include only supported tactics/techniques. Use as many bullet points as needed based on the evidence, and say when confidence is low.\n"
        "- Recommended Actions: Include only the actions supported by the evidence. Do not force a fixed number of bullets. Start each bullet with one priority tag: [Immediate], [Next], or [Monitor], and make each action specific and operational.\n"
